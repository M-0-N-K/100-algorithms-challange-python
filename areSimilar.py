'''
wo arrays are called similar if one can be obtained from another by swapping at most one pair 
...
def areSimilar(arr1,arr2):
    ...
    if len(arr1)!=len(arr2):
        return False
    while swaps<2 and index<len(arr1):
        if arr1[index]!=arr2[index]:
            if arr2[index] in arr1[index:]:
                swaps+=1
                temp=arr1[index]
                loc=arr1.index(arr2[index])
                arr1[index]=arr1[loc]
                arr1[loc]=temp
            else:
                return False
